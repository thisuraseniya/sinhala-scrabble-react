...
export default class OnlinePlayerRack extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidUpdate(prevProps, prevState) {
    // update rack online
    if (
      this.props.dbData.joinerRack &&
      this.props.dbData.joinerRack !== prevProps.dbData.joinerRack &&
      this.state.player2Rack === null
    ) {
      let joinerRack = this.props.dbData.joinerRack;
      console.log("updating joiner rack");
      this.setState({
        player2Rack: joinerRack,
        player2RemovedRack: joinerRack,
        player2PlayingRack: joinerRack.slice(0, rackLength),
        player2TempRack: joinerRack.slice(0, rackLength),
      });
    }
  ...
  dealLettersToPlayers = () => {
    ...
    if (this.props.type === "host") {
      try {
        db.collection("scrabble_online").doc(this.props.dbData.host).update({
          hostRack: p1,
          joinerRack: p2,
        });
      } catch (error) {
  ...
  render() {
    ...
    const { currentplayer, type } = this.props;

    return (
      <>
        {currentplayer === 1 && type === "host" ? (
          <>
            <div className="player-text mb-2">It's your turn</div>
            <table>
              <tbody>
                <tr>
                  {player1PlayingRack &&
